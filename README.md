...
- **404.**
- **Mobile** — `core/navigation`'s built-in `overlayMenu: "mobile"` renders the hamburger and overlay; the overlay shows the same inner blocks as the desktop nav (Jobs / FAQ / Feedback + the "Post a Job" CTA pattern).
...
- **`menu-toggle`** + `parts/mobile-overlay.html` + `js/mobile-overlay.js` → `core/navigation` with `overlayMenu: "mobile"` and the `header-cta` pattern moved inside the navigation block (so it appears in both the desktop nav and the mobile overlay). Core renders the hamburger, animates the overlay, and handles a11y. WordPress 7.0's [Customisable Navigation Overlays](https://github.com/WordPress/gutenberg/issues/73084) initiative will let themes ship a starter overlay template part edited via the Navigation block's sidebar; when that lands, this theme can opt-in by adding an overlay pattern without re-introducing custom toggle markup.
